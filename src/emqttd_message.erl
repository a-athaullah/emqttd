...
make(From, Qos, Topic, Payload) ->
    #mqtt_message{id        = msgid(),
                  ...
                  qos       = ?QOS_I(Qos),
                  topic     = Topic,
                  payload   = case {string:str(binary_to_list(Topic),"/s")==string:length(binary_to_list(Topic))-1,string:length(NewPayload)==1} of 
                                {true,true} -> 
                                    list_to_binary(string:concat(string:concat(binary_to_list(NewPayload),":"),integer_to_list(round(erlang:system_time() / 1.00e6)))); 
                                OtherWise-> Payload 
                               end,
...
from_packet(#mqtt_packet_connect{client_id   = ClientId,
    ...
    #mqtt_message{id        = msgid(),
                  ...
                  qos       = Qos,
                  dup       = false,
                  payload   = list_to_binary(string:concat("0:",integer_to_list(round(erlang:system_time() / 1.000e6)))), 
                  timestamp = os:timestamp()}.
